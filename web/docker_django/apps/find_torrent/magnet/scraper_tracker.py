...
def scrape(tracker, hashes):
    ...
    if parsed.scheme == "udp":
        return scrape_udp(parsed, hashes, tracker_name)
    ...
    if parsed.scheme in ["http", "https"]:
        ...
        return scrape_http(parsed, hashes, tracker_name)

    raise RuntimeError("Unknown tracker scheme: %s" % parsed.scheme)


def scrape_udp(parsed_tracker, hashes, tracker_name):
    ...
    sock.sendto(req, conn)
    buf = sock.recvfrom(2048)[0]
    return udp_parse_scrape_response(buf, transaction_id, hashes, tracker_name)


def scrape_http(parsed_tracker, hashes, tracker_name):
    ...
    for hash, stats in decoded['files'].iteritems():
        ...
        p = stats["incomplete"]
        c = stats["downloaded"]
        ret[str(nice_hash).upper()] = {"seeds": s, "peers": p, "complete": c, "tracker": tracker_name}
    return ret
...
def udp_parse_scrape_response(buf, sent_transaction_id, hashes, tracker_name):
    if len(buf) < 16:
    ...
    if action == 0x2:
        ...
        for hash in hashes:
            ...
            offset += 4
            ret[hash] = {"tracker": tracker_name, "seeds": seeds, "peers": leeches, "complete": complete}
        return ret
    elif action == 0x3:
