...
def magnet2torrent(magnet, output_name=None):
    ...
    while not handle.has_metadata():
        ...
        if time() - t0 > 10:
            ...
            break
    ses.pause()
    print(len(handle.get_peer_info()))
    # ses.pause()
    print("Done")
    ...
    # for i in range(10*2):
    #     print(len(handle.get_peer_info()))
    #     peer_info.append(len(handle.get_peer_info()))
    #     s = handle.status()
    #     print(s.num_peers, s.num_seeds)
    #     peers.append(s.num_peers)
    #     seeds.append(s.num_seeds)
    #
    #     print('='*80)
    #     sleep(3)
    # print(sum(peers) + max(peer_info), sum(seeds), sum(peer_info))
    ...
    return torinfo.name(), t_filee  # output
